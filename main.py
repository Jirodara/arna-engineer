...
SECRETS_FILE   = MEMORY_DIR / 'secrets.json'
...
def load_versions():
    ...
    versions.sort(key=lambda v: v.get('ts', 0), reverse=True)
    return versions
...
def analyze_file(name, content):
    ...
    if name.endswith('.py'):
        # 1. Parantez dengesi — string ve yorum satırlarını hariç tut
        clean = re.sub(r'#[^\n]*', '', content)           # yorumları kaldır
        clean = re.sub(r'"""[\s\S]*?"""', '', clean)      # docstring kaldır
        clean = re.sub(r"'''[\s\S]*?'''", '', clean)      # docstring kaldır
        clean = re.sub(r'"[^"\\]*(?:\\.[^"\\]*)*"', '', clean)  # string kaldır
        clean = re.sub(r"'[^'\\]*(?:\\.[^'\\]*)*'", '', clean)  # string kaldır
        opens  = clean.count('(')
        closes = clean.count(')')
        if opens != closes:
    ...
    if name.endswith('.html'):
        ...
        # 3. Duplicate HTML ID kontrolü ve otomatik düzeltme
        ids = re.findall(r'id=["\']([^"\']+)["\']', fixed)
        seen = set()
        dup_ids = []
        for id_ in ids:
            if id_ in seen:
                ...
                dup_ids.append(id_)
            seen.add(id_)
        # Otomatik düzelt — ikinci görünümü kaldır
        if dup_ids:
            for dup_id in dup_ids:
                matches = list(re.finditer(r'id=["\']' + re.escape(dup_id) + r'["\']', fixed))
                if len(matches) >= 2:
                    for m in reversed(matches[1:]):
                        fixed = fixed[:m.start()] + f'data-dup-removed="{dup_id}"' + fixed[m.end():]
                    fixes.append(f'Duplicate ID #{dup_id} düzeltildi')
...
# ════════════════════════════════════════════════
# CROSS-FILE OTOMATİK DÜZELTİCİ
# ════════════════════════════════════════════════

def auto_fix_cross_file(project):
    """Dosyalar arası uyumsuzlukları otomatik düzelt"""
    fixes_applied = []
    files = project['files']

    # 1. priceScore uyumsuzluğu — bot.py'yi kaynak al, index.html'i güncelle
    if 'bot.py' in files and 'index.html' in files:
        bot_ps = re.search(r'def price_score[^:]+:.*?return\s+(\d+)', files['bot.py'], re.DOTALL)
        idx_ps = re.search(r'function\s+priceScore[^{]+\{[^}]*return\s+(\d+)', files['index.html'], re.DOTALL)
        if bot_ps and idx_ps:
            bot_val = bot_ps.group(1)
            idx_val = idx_ps.group(1)
            if bot_val != idx_val:
                # index.html'deki priceScore return değerini güncelle
                old_idx = idx_ps.group(0)
                new_idx = old_idx.replace(f'return {idx_val}', f'return {bot_val}')
                files['index.html'] = files['index.html'].replace(old_idx, new_idx)
                fixes_applied.append({
                    'file': 'index.html',
                    'msg': f'priceScore düzeltildi: {idx_val} → {bot_val} (bot.py ile eşitlendi)'
                })

    # 2. Duplicate HTML ID — tüm HTML dosyalarında
    for name, content in list(files.items()):
        ...
        for id_ in ids_found:
            if id_ in seen:
                # İkinci görünümü kaldır
                pat = r'id=["\']' + re.escape(id_) + r'["\']'
                matches = list(re.finditer(pat, content))
                if len(matches) >= 2:
                    for m in reversed(matches[1:]):
                        content = content[:m.start()] + f'data-was-id="{id_}"' + content[m.end():]
                    files[name] = content
                    fixes_applied.append({
                        'file': name,
                        'msg': f'Duplicate ID #{id_} düzeltildi'
                    })
            seen.add(id_)

    # 3. Bot port — index.html'de bot portu yoksa ekle
    if 'bot.py' in files and 'index.html' in files:
        bot_port_m = re.search(r"HTTPServer\([^,]+,\s*(\d+)", files['bot.py'])
        if bot_port_m:
            bot_port = bot_port_m.group(1)
            if bot_port not in files['index.html'] and '/api' in files['index.html']:
                # Portu yoruma ekle
                files['index.html'] = files['index.html'].replace(
                    "// ARNA Bot API",
                    f"// ARNA Bot API — Port {bot_port}"
                )

    return fixes_applied, files
    ...
    def do_GET(self):
        ...
        if self.path.startswith('/download'):
            try:
                ...
                v_id   = qs.get('version_id', [''])[0]
                versions = load_versions()
                ver    = next((v for v in versions if v['id'] == v_id), None)
                files  = ver['files'] if ver else load_project()['files']
                label  = ver['label'] if ver else 'latest'
                buf    = io.BytesIO()
                ...
                self.send_header('Content-Disposition', f'attachment; filename=arna-{label}.zip')
                self.send_header('Access-Control-Allow-Origin', '*')
                ...
                self.end_headers()
                self.wfile.write(f'Hata: {e}'.encode())
            return
        ...
        if self.path == '/api/state':
            with _lock:
                ...
                self._json({
                    'files':    list(project['files'].keys()),
                    'meta':     project['meta'],
                    'versions': [{'id':v['id'],'label':v['label'],'date':v['date'],'desc':v['description'],
        ...
        if self.path == '/api/secrets':
            self._json(load_secrets())
            return

        if self.path == '/eng-manifest.json':
            ...
            m = _json.dumps({
                ...
                "theme_color": "#060a0f",
                "icons": [{"src": "/eng-icon.png", "sizes": "192x192", "type": "image/png"}]
            }).encode()
            self.send_response(200)
        ...
        if self.path == '/eng-sw.js':
            self.send_response(200)
            self.send_header('Content-Type', 'application/javascript')
            self.end_headers()
            self.wfile.write(b"self.addEventListener('fetch',()=>{});")
            return

        if self.path == '/eng-icon.png':
            try:
                icon = (BASE_DIR / 'eng-icon.png').read_bytes()
                self.send_response(200)
    ...
    def do_POST(self):
        ...
        with _lock:
            ...
            if action == 'upload_files':
                ...
                # Otomatik snapshot — değişiklik varsa kaydet
                all_changed = changed + added
                if all_changed:
                    auto_desc = f"Otomatik — {', '.join(all_changed[:3])}"
                    if len(all_changed) > 3:
                        auto_desc += f" +{len(all_changed)-3} daha"
                    save_snapshot(project, auto_desc, changed, added)
            ...
            # ── GITHUB PUSH ──
                ...
                repo    = cmd.get('repo', '').strip()
                message = cmd.get('message', 'ARNA Engineer — otomatik güncelleme')

                # Kayıtlı secrets'tan al (boşsa)
                secrets = load_secrets()
                if not token:
                    token = secrets.get('gh_token', '')
                if not repo:
                    repo = secrets.get('gh_repo', '')

                if not token or not repo:
                    self._json({'error': 'Token ve repo gerekli'})
                    return

                # Token ve repo'yu sunucuda kaydet (güncelle)
                secrets['gh_token'] = token
                secrets['gh_repo']  = repo
                save_secrets(secrets)

                project  = load_project()
                versions = load_versions()
                cur_ver  = versions[0]['label'] if versions else 'latest'
                ...
                for filename, content in project['files'].items():
                    try:
                        ...
                        sha = None
                        try:
                            req = ur.Request(f'{api_base}/{filename}', headers=headers)
                            res = ur.urlopen(req, timeout=10)
                            existing = json.loads(res.read())
                            sha = existing.get('sha')
                        except Exception:
                            pass  # Yeni dosya, SHA yok

                        # Dosyayı yükle
                        payload = {
                            'message': f'{message} ({cur_ver}) — {filename}',
                            'content': base64.b64encode(content.encode('utf-8', errors='replace')).decode('ascii'),
                        }
                        if sha:
                            payload['sha'] = sha

                        data = json.dumps(payload).encode('utf-8')
                        req  = ur.Request(f'{api_base}/{filename}', data=data, headers=headers, method='PUT')
                        ...
                        errors.append({'file': filename, 'error': f'HTTP {e.code}: {err_body}'})
                    except Exception as e:
                        errors.append({'file': filename, 'error': str(e)[:150]})

                # Push başarılıysa otomatik snapshot al
                if pushed:
                    checksums = load_checksums()
                    changes = detect_changes(project, checksums)
                    changed_list = [n for n,c in changes.items() if c['type'] == 'modified']
                    added_list   = [n for n,c in changes.items() if c['type'] == 'new']
                    push_desc = f"GitHub push — {', '.join(pushed[:3])}"
                    if len(pushed) > 3:
                        push_desc += f" +{len(pushed)-3} daha"
                    snap = save_snapshot(project, push_desc, changed_list, added_list)
                    for name, content in project['files'].items():
                        checksums[name] = checksum(content)
                    save_checksums(checksums)
                    new_ver = snap['label']
            ...
            # ── TAM OTOMATİK DÜZELT ──
                ...
                project = load_project()
                # 1. Her dosyayı kendi içinde düzelt
                file_fixes = {}
                for name, content in list(project['files'].items()):
                    result = analyze_file(name, content)
                    if result['fixed_content']:
                        project['files'][name] = result['fixed_content']
                        file_fixes[name] = result['fixes']

                # 2. Cross-file düzeltmeler
                cross_fixes, project['files'] = auto_fix_cross_file(project)

                project['last_updated'] = now_str()
                save_project(project)

                all_fixes = []
                for name, fixes in file_fixes.items():
                    for f in fixes:
                        if isinstance(f, dict):
                            all_fixes.append(f)
                        else:
                            all_fixes.append({'file': name, 'msg': str(f)})
                for f in cross_fixes:
                    if isinstance(f, dict):
                        all_fixes.append(f)
                    else:
                        all_fixes.append({'file': '?', 'msg': str(f)})
